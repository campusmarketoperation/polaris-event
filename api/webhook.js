...
export default async function handler(req, res) {
  ...
  // 決済完了イベントを処理（カード・PayPay）
  if (event.type === 'checkout.session.completed') {
    const session = event.data.object;

    // コンビニ払いは入金確認待ちなのでスキップ
    if (session.payment_status === 'unpaid') {
      console.log('コンビニ払い番号発行（入金待ち）:', session.id);
      return res.status(200).json({ received: true });
    }

    const email = session.customer_details?.email || session.customer_email;
    console.log('決済完了（即時）:', email, session.id);
    await notifyGAS(gasUrl, email, session.id);
  }
  ...
  if (event.type === 'checkout.session.async_payment_succeeded') {
    const session = event.data.object;
    const email   = session.customer_details?.email || session.customer_email;
    console.log('コンビニ払い入金確認:', email, session.id);
    await notifyGAS(gasUrl, email, session.id);
  }
...
// GASに支払い完了を通知
async function notifyGAS(gasUrl, email, sessionId) {
  if (!email || !gasUrl) return;
  try {
